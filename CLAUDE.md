...
6. `settings/<アプリ名>/<ダッシュボード名>.json` に**固定名で上書き保存**する(履歴は git の diff で追う。
   アプリが 1 つだけなら `settings/<アプリ名>-<ダッシュボード名>.json` のフラット置きでもよい)
7. 反映方法(インポートかビューへ取り込むか)と確認ポイントを利用者に伝える
...
- kintone への書き込みは行わない(`ksql_mutate` 禁止。認証は閲覧のみのトークンを想定)
