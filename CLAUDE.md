...
7. 保存先は**利用者が指定したファイル名に従う**(ファイル名の体系は利用者の管理)。
   指定が無ければ、まず settings/ の既存構成を見る —
   **同一アプリでフラット置き(`settings/<アプリ名>-<名前>.json`)とフォルダー置き
   (`settings/<アプリ名>/<名前>.json`)を混在させない。既存の形式に合わせる。**
   既存が無ければ `settings/<アプリ名>-<ダッシュボード名>.json` を提案する。
