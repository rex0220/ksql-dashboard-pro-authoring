...
- 版数: 1.3(2026-08-02。**エンジン 3.39 系 / 設定ファイル仕様 1.13 に追従** —
  ①`LAPP_<名前>` が解禁(`logicalApps` を定義すれば使える。環境非依存設定の推奨手段)
  ②SELECT 別名の英字は**結果列名では小文字**(表示は書いたとおり。`columns` / `totals` /
  `mapping` の参照は小文字で書く)③期間コントロール(`controls` + `@変数`)
  ④取得上限・一時テーブル上限が 50,000 へ ⑤新オプション群(行番号・単位を見出しに・
  リンク・説明・横積層棒など)は設定ファイル仕様 §5〜6 を参照)
- 1.2(2026-07-31。**出力は封筒形式が必須**というルールを追加(K-81 の封筒形式・K-39 E2E の知見)。
  素の config だけの生成は誤りと明記)
- 1.1(2026-07-28。**エンジン v3.33.0 / K-33 / K-26b に合わせて更新** —
  複文と一時テーブルがペインで使えるようになり、検証と反映の手順が変わった)
- 1.0(2026-07-27。MCP の疎通・ツール一覧は実機で確認済み)
- 関連: [設定ファイル仕様](./設定ファイル仕様.md)(フォーマットの正本)/
  [ダッシュボードレシピ集](./ダッシュボードレシピ集.md)(SQL の書き方)/
  課題台帳(issues ブランチ)K-26 / K-39
